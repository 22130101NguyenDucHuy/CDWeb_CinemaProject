...
export default function Home() {
  ...
  const isFullSeries = (movie) => {
    ...
    return v.includes('full') || v.includes('hoan') || v.includes('hoÃ n') || v.includes('tron') || v.includes('trá»n');
  };
  ...
  // Láº¥y poster cho dáº£i thumbnail hero
  useEffect(() => {
  ...
  // Láº¥y poster_url tá»« API chi tiáº¿t phim cho billboard
  useEffect(() => {
  ...
  return (
    <div className="home-page">
        ...
        <header className="billboard" style={{ backgroundImage: `url(${billboardImg})` }}>
          ...
          <div className="billboard-info">
            ...
            <p className="billboard-desc">
              Má»™t tÃ¡c pháº©m tá»« {(activeHeroMovie.country || []).map(c => c.name).join(', ')}.
              {' '}Thá»ƒ loáº¡i: {(activeHeroMovie.category || []).map(c => c.name).join(', ')}.
              {' '}NÄƒm {activeHeroMovie.year}.
            </p>
            <div className="billboard-buttons">
              <Link href={`/phim/${activeHeroMovie.slug}`} className="btn btn-play">
                <Play size={24} fill="currentColor" /> PhÃ¡t
              </Link>
              <Link href={`/phim/${activeHeroMovie.slug}`} className="btn btn-more">
                <Info size={24} /> ThÃ´ng Tin KhÃ¡c
              </Link>
            </div>
          </div>
      ...
      {/* Section 1: PHIM CHIáº¾U Ráº P Má»šI */}
      <MovieSection
        title="PHIM CHIáº¾U Ráº P Má»šI"
      ...
      <MovieSection
        title="PHIM Bá»˜"
        movies={phimBoMovies}
        tabs={['HÃ n Quá»‘c', 'Trung Quá»‘c', 'Ã‚u - Má»¹', 'Phim Bá»™ Full']}
        cdnUrl={cdnUrl}
        selectedTab={selectedSeriesTab}
        onTabChange={setSelectedSeriesTab}
        filterFn={(movie, tab) => {
          if (!tab) return true;
          if (tab === 'Phim Bá»™ Full') return isFullSeries(movie);
          if (tab === 'Ã‚u - Má»¹') return hasCountry(movie, 'Ã‚u Má»¹');
      ...
      <MovieSection
        title="PHIM Láºº"
        movies={phimLeMovies}
        tabs={['HÃ nh Äá»™ng', 'Kinh Dá»‹', 'HÃ i HÆ°á»›c']}
        cdnUrl={cdnUrl}
      ...
      {/* Section 4: PHIM HOáº T HÃŒNH */}
      <MovieSection
        title="PHIM HOáº T HÃŒNH"
