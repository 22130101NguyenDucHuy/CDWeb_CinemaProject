...
const Navbar = () => {
  ...
  // ÄÃ³ng dropdown khi chuyá»ƒn trang
  useEffect(() => {
    setActiveDropdown(null);
    setShowSearch(false);
    setShowSuggestions(false);
  ...
  // Gá»£i Ã½ tÃ¬m kiáº¿m (AJAX) - giá»›i háº¡n 5 phim
  useEffect(() => {
    ...
    <nav className={`navbar ${isScrolled ? 'scrolled' : ''}`} ref={navRef}>
      <div className="nav-left">
        ...
        <ul className="nav-links">
          <li><Link href="/" className={pathname === '/' ? 'active' : ''}>Trang Chá»§</Link></li>
          ...
          <li className="dropdown">
            ...
            >
              Danh SÃ¡ch <ChevronDown size={14} className="chevron" />
            </button>
            {activeDropdown === 'list' && (
          ...
          <li className="dropdown">
            ...
            >
              Thá»ƒ Loáº¡i <ChevronDown size={14} className="chevron" />
            </button>
          ...
          <li className="dropdown">
            ...
            >
              Quá»‘c Gia <ChevronDown size={14} className="chevron" />
            </button>
      ...
      <div className="nav-right">
        {showSearch ? (
            ...
            {showSuggestions && (suggestions.length > 0 || isSuggesting) && (
              <div className="search-suggestions" role="listbox">
                {isSuggesting && suggestions.length === 0 ? (
                  <div className="search-suggestion muted">Äang tÃ¬m...</div>
                ) : (
                  suggestions.map((movie) => {
                    ...
                    return (
                      <div key={movie?._id || movie?.slug} className="search-suggestion-wrapper">
                        ...
                        >
                          <span className="search-suggestion-title">{movie?.name || 'KhÃ´ng cÃ³ tÃªn'}</span>
                          {movie?.year ? (
                        ...
                        <div className="search-suggestion-preview">
                           ...
                           <div className="search-preview-info">
                             ...
                             <div className="preview-meta">
                               ...
                               {movie?.year && <span>NÄƒm: {movie.year}</span>}
                               {movie?.quality && <span>Cháº¥t lÆ°á»£ng: {movie.quality}</span>}
                             </div>
                           </div>
                        </div>
                      </div>
          ...
          <button className="icon-btn" onClick={() => setShowSearch(true)} aria-label="TÃ¬m kiáº¿m">
            <Search size={22} />
          </button>
        )}
      </div>
